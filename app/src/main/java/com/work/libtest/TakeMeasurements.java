...
//                        processIncomingData(newBytes);
                    try {
                        ...
                        Log.e(TAG, "Exception thrown in processing new reading: " + e);
                    }
                    break;
                }
...
//            textAcceptResultDip.setText("");
    ...
    private void processNewReading() {
        ...
        try {
            ...
            DecimalFormat numberFormatShort = new DecimalFormat("#.0");
        ...
        //Check D-Intgrity stuff
        /*
        D* - Dip outside expected range
        M* - Total magnetic field outside excpected range
        A* - Azimuth outside expected range
        # - probe outside operating temperature
        ! - probe moving during survey shot
         */
        String DIntegrity = ""; //set default integrity to nothing
        if ((magMag > Globals.simplePreferences.getNominalValueMagneticField() + Globals.simplePreferences.getMaximumDeviationMagneticField() || (magMag < Globals.simplePreferences.getNominalValueMagneticField() - Globals.simplePreferences.getMaximumDeviationMagneticField()))) {
            DIntegrity = DIntegrity + "M*";
        }
        if (Integer.valueOf(newMeasurementBeingCollected.getTemp()) > 70) {
            DIntegrity = DIntegrity + "#";
        }
        DecimalFormat numberFormatShort = new DecimalFormat("#.0");
        ...
        DetailedMeasurement newDetailedMeasurement = new DetailedMeasurement(newMeasurementBeingCollected, String.valueOf(recordedShots.size()),
                ...
                DIntegrity, numberFormatShort.format(magMag).toString());
...
//                        progressBar.setVisibility(ProgressBar.INVISIBLE);                           //Hide the circular progress bar
    ...
    public void measurementClick(View view) throws InterruptedException {
        if (!probeCollectingData) {
            boolean on = probeOnMode();
            if (on) {
                collectMeasurement();
            } else {
                Log.e(TAG, "Couldn't turn on");
            }
        } else {
            collectMeasurement();
        }
