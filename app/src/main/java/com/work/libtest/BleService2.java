...
public class BleService2 extends Service {
    private final static String TAG = "BLE SERVICE 2";                             //Service name for logging messages on the ADB


    //Changed from the returns by BleService so that the app stops confusing the services
    private final static String ACTION_ADAPTER_STATE_CHANGED_WHITE = "android.bluetooth.adapter.action.STATE_CHANGED";           //Identifier for Intent that announces a change in the state of the Bluetooth radio
    private final static String EXTRA_ADAPTER_STATE_WHITE = "android.bluetooth.adapter.extra.STATE";                    //Identifier for Bluetooth connection state attached to state changed Intent
    public final static String ACTION_BLE_CONNECTED_WHITE = "com.example.javatest.ACTION_BLE_CONNECTED";         //Identifier for Intent to announce that a BLE device connected
    public final static String ACTION_BLE_DISCONNECTED_WHITE = "com.example.javatest.ACTION_BLE_DISCONNECTED";      //Identifier for Intent to announce that a BLE device disconnected
    public final static String ACTION_BLE_DISCOVERY_DONE_WHITE = "com.example.javatest.ACTION_BLE_DISCOVERY_DONE";    //Identifier for Intent to announce that service discovery is complete
    public final static String ACTION_BLE_DISCOVERY_FAILED_WHITE = "com.example.javatest.ACTION_BLE_DISCOVERY_FAILED";  //Identifier for Intent to announce that service discovery failed to find the service and characteristics
    public final static String ACTION_BLE_CONFIG_READY_WHITE = "com.example.javatest.ACTION_BLE_CONFIG_READY"; //Identifier for Intent to announce a new characteristic notification
    ...
    public IBinder onBind(Intent intent) {
        Log.i(TAG, "Binding to BleService2");
        try {
            registerReceiver(broadcastReceiver, new IntentFilter(BleService2.ACTION_ADAPTER_STATE_CHANGED_WHITE)); //Register receiver to handle Intents from the BluetoothAdapter
    ...
    private final BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            try {
                ...
                if (BleService2.ACTION_ADAPTER_STATE_CHANGED_WHITE.equals(action)) {                       //See if BluetoothAdapter has broadcast that its state changed
                    Log.d(TAG, "BluetoothAdapter state changed");
                    final int stateAdapter = intent.getIntExtra(BleService2.EXTRA_ADAPTER_STATE_WHITE, 0); //Get the current state of the adapter
                    switch (stateAdapter) {
                ...
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    ...
                    switch (newState) {
                        case BluetoothProfile.STATE_CONNECTED: {                                    //Are now connected
                            ...
                            sendBroadcast(new Intent(ACTION_BLE_CONNECTED_WHITE));                        //Let the MainActivity know that we are connected by broadcasting an Intent
                        ...
                        case BluetoothProfile.STATE_DISCONNECTED: {                                 //Are now disconnected
                            ...
                            sendBroadcast(new Intent(ACTION_BLE_DISCONNECTED_WHITE));                     //Let the MainActivity know that we are disconnected by broadcasting an Intent
                        }
                    }
 ...
 */
                ...
                if (!discoveryFailed) {                                                             //Service discovery returned the correct service and characteristics
                    ...
                    sendBroadcast(new Intent(ACTION_BLE_DISCOVERY_FAILED_WHITE));                         //Broadcast Intent to announce the failure of service discovery
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) { //Received notification or indication with a new value for a characteristic
            ...
            try {
                ...
                if (UUID_BORE_SHOT_CHAR.equals(characteristic.getUuid())) {               //See if it is the Transparent Receive characteristic (the only notification expected)
                    ...
                    processBoreshot();
                    sendBroadcast(new Intent(ACTION_BLE_NEW_DATA_RECEIVED_WHITE));                        //Broadcast Intent to announce the new data. This does not send the data, it needs to be read by calling readFromTransparentUART() below
                }
            ...
            try {
                ...
                if (UUID_PROBE_MODE_CHAR.equals(characteristic.getUuid())) {               //See if it is the Transparent Receive characteristic (the only notification expected)
                    ...
                    if (stateInterrogateConfig == statesInterrogateConfig.WAITING_DEBUG2) {
                        ...
                        sendBroadcast(new Intent(ACTION_BLE_FETCH_CAL_WHITE));
                        //sendBroadcast(new Intent(ACTION_BLE_CONFIG_READY));  // PJH - finished (for now) - in progress
                    }
                    ...
                    if (stateInterrogateConfig == statesInterrogateConfig.RETRIEVE_CAL) {
                        ...
                        if (calPageNumber[0] < last_page) {
                            ...
                            sendBroadcast(new Intent(ACTION_BLE_CONFIG_READY_WHITE));  // PJH - finished (for now) - in progress
